...
class EKFSLAM():
    ...
    def predict_location(self, delta_x, delta_y, delta_theta):
        ...
        # motion's noise
        R = np.diag([self.x_error, self.y_error, self.theta_error])

        # predict robot's pose covariance
        self.covariance = G @ self.covariance @ G.T + Fx.T @ R @ Fx

    def correct_location(self, observation):
        ...
        for k in range(1, self.Nt + 1):
            ...
            # measurement's noise
            Q = np.diag([self.range_error * z[k][0][0], self.bearing_error])

            # innovation covariance
            psi[k] = H[k] @ self.covariance @ H[k].T + Q
        ...
        for i in range(1, observation.shape[1] + 1):
            ...
            # measurement's noise
            Q = np.diag([self.range_error * z[self.Nt + 1][0][0], self.bearing_error])

            # innovation covariance
            psi[self.Nt + 1] = H[self.Nt + 1] @ self.covariance @ H[self.Nt + 1].T + Q
            ...
            for k in range(1, self.Nt + 1):
                pi[k] = ((zt - z[k]).T @ np.linalg.inv(psi[k]) @ (zt - z[k]))[0][0]
...
# robot = EKFSLAM(max_landmark=2, x_error=0.1, y_error=0.1, theta_error=0.1, range_error=0.01, bearing_error=np.deg2rad(1), threshold=5.991)
# delta_x = 10
# delta_y = 10
# delta_theta = 0.1
# observation = np.array(([[500],[500]]))
# robot.predict_location(delta_x, delta_y, delta_theta)
# robot.correct_location(observation)
# observation = np.array(([[506],[506]]))
# robot.predict_location(delta_x, delta_y, delta_theta)
# robot.correct_location(observation)
# print(robot.mean)
