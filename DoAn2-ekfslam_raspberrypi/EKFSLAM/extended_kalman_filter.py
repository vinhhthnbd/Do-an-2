...
class EKFSLAM():
    def __init__(self,maximum_landmark,odo_c,mea_mm,mea_rad):
        ...
        self.Nt=0 # current known landmark
        ...
        self.Mahanobis_threshold=5.991#we have this threshold constant define from Chi square distribution table (which 2 dregee of freedom and 5%) 
    ...
    def prediction(self):
        ...
        Gt=np.eye(2*self.maximum_landmark+3,2*self.maximum_landmark+3) + (self.Fx.T)@Gtx@self.Fx
        ##
        ...
        self.covariance=Gt@self.covariance@(Gt.T)+(self.Fx.T)@Rt@self.Fx#(2N+3)*(2N+3) matrix
        
    #
    def correction(self,z):
        # z:this matrix contains information about the landmark we observed from lidar
        #z :2*x matrices(x: number of extracted landmarks)(this matrix contains coordinates of landmark after we use RANSAC algorithm)
        ...
        for k in range(self.Nt):
            ...
            Fx_j=np.hstack([[np.eye(5,3)],[np.zeros(5,2*(k+1)-2)],[np.vstack([[np.zeros((3,2))],[np.eye(2,2)]])],[np.zeros((5,2*self.maximum_landmark-2*(k+1)))]])
            Ht=(1/q)*np.array([[-np.sqrt(q)*delta_x,-np.sqrt(q)*delta_y,0,np.sqrt(q)*delta_x,np.sqrt(q)*delta_y],
                        [delta_y,-delta_x,-q,-delta_y,delta_x]])@Fx_j
            #Qt
            Qt=np.array([[np.power(z_hat_t*self.mea_mm,2),0],[0,np.power(self.mea_rad,2)]])

            #find saved landmarks’ covariance1
            si_t=Ht@self.covariance@(Ht.T)+Qt#landmark covariance
            si.append(si_t)
            H.append(Ht)
        #endfor
        ...
        for i in range(z.shape[1]):
            ...
            Ht=(1/q)*np.array([[-np.sqrt(q)*delta_x,-np.sqrt(q)*delta_y,0,np.sqrt(q)*delta_x,np.sqrt(q)*delta_y],
                        [delta_y,-delta_x,-q,-delta_y,delta_x]])@Fx_j# this is a 2*(2N+3) matrix
            ...
            self.covariance=(np.eye(2*self.maximum_landmark+3,2*self.maximum_landmark+3)-K@H[j])@self.covariance
        #end
